...
- Added new definition ScriptableObjects that get generated by the project importer
  - All definition values from the entire json structure are contained in these objects
  - All components in the imported hierarchy will contain the appropriate references for ease of accessing all data without any deserializing required.
  - Many composite values in the json are converted to the appropriate fields:
    - X+Y values into Vector2 or Vector2Int
    - Color strings/ints into the Color struct
    - Uid references into proper ScriptableObject references
    - Tileset rectangles referencing the appropriate tileset definition file's tile
