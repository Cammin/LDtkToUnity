...
- Changed how the project importer manages assets
  ...
  - All potential artifact assets are generated instead of only the ones needed.
    - This is to ensure that any possible sprites/tiles that separate levels may need are available.
    - This will negatively impact the sprite atlas sizes, but this memory optimisation will return in the future.
...
- Minimum supported Unity version raised to 2020.1 (From 2019.3)
  - The version had to be raised due to new APIs being used to improve import speed
...
- The json asset artifact will no longer have it's level's layer instances available if using separate level files for performance reasons. Instead, deserialize the specific separate levels to get a level's data.
